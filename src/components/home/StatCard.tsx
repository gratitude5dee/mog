...
export const StatCard = ({ icon, label, value, trend, trendDirection = 'neutral', className, index = 0 }: StatCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <p className="text-[10px] text-muted-foreground/70 mb-1 font-semibold uppercase tracking-[0.15em]">{label}</p>
        <div className="flex items-baseline gap-3 mt-2">
          {isNumericValue ? (
            <NumberTicker value={value} delay={0.3 + index * 0.1} className="text-3xl font-bold text-foreground tracking-tight" />
          ) : (
            <span className="text-3xl font-bold text-foreground tracking-tight">{value}</span>
          )}
        </div>
        {trend && (
          <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + index * 0.1 }} className="mt-3">
            <span className={cn(
