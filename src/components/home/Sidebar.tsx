import { Grid, Users, Globe, Star, ChevronLeft, LogOut, Layers, Sparkles, FolderKanban } from 'lucide-react';
...
export const Sidebar = ({ activeView, onViewChange }: SidebarProps) => {
  ...
  const mainNavItems = [
    ...
    { id: 'feed', label: 'Feed', icon: Layers, isRoute: true, showBadge: true },
  ];
  ...
  const NavItem = ({ item, index }: { item: typeof mainNavItems[0]; index: number }) => {
    ...
    const content = (
      ...
      >
        {isActive && <ShineBorder shineColor="#8B5CF6" borderWidth={1} duration={10} />}
        <div className={cn(
          "w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 flex-shrink-0",
          isActive ? "bg-accent-purple/15 shadow-sm" : "bg-white/[0.04]"
        )}>
          <Icon className={cn("w-4 h-4", isActive && "text-accent-purple")} />
        </div>
        <AnimatePresence mode="wait">
          {!isCollapsed && (
            <motion.span variants={textVariants} initial="hidden" animate="visible" exit="hidden" transition={{ duration: 0.15 }} className="flex-1 text-left whitespace-nowrap">
              {item.label}
            </motion.span>
          )}
        </AnimatePresence>
        ...
        <Tooltip delayDuration={0}>
          ...
          <TooltipContent side="right" className="font-medium">
            <span className="flex items-center gap-2">
              {item.label}
              {item.showBadge && (
                <Badge variant="secondary" className="text-[9px] bg-accent-purple/15 text-accent-purple border-accent-purple/20 px-1.5 py-0.5">New</Badge>
              )}
            </span>
          </TooltipContent>
        </Tooltip>
    ...
    const content = (
      ...
      >
        <div className={cn(
          ...
          isActive ? "bg-accent-purple/15" : "bg-white/[0.04]"
        )}>
          <Icon className={cn("w-4 h-4", isActive && "text-accent-purple")} />
        </div>
        <AnimatePresence mode="wait">
          {!isCollapsed && (
            <motion.span variants={textVariants} initial="hidden" animate="visible" exit="hidden" transition={{ duration: 0.15 }} className="whitespace-nowrap">
              {item.label}
            </motion.span>
          )}
        </AnimatePresence>
    ...
    <TooltipProvider>
      ...
      >
        <div className="absolute inset-0 opacity-0 group-hover/sidebar:opacity-100 transition-opacity duration-500 pointer-events-none rounded-r-xl overflow-hidden">
          <ShineBorder shineColor={["hsl(var(--accent-purple))", "hsl(var(--amber))"]} borderWidth={1} duration={8} />
        </div>
        ...
        <nav className={cn("relative z-10 flex-1 space-y-6 overflow-y-auto", isCollapsed ? "p-2" : "p-4")}>
          <div>
            <AnimatePresence mode="wait">
              {!isCollapsed && (
                <motion.div variants={textVariants} initial="hidden" animate="visible" exit="hidden" className="flex items-center gap-2 px-3 mb-3">
            ...
            <Tooltip delayDuration={0}>
              <TooltipTrigger asChild>
                <button
                  ...
                  className={cn(
                    "w-full flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors",
                    isCollapsed && "justify-center px-2"
                  )}
                >
                  ...
                  <AnimatePresence mode="wait">
                    {!isCollapsed && (
                      <>
                        <motion.span variants={textVariants} initial="hidden" animate="visible" exit="hidden" className="flex-1 text-left font-medium whitespace-nowrap">
                          Favorites
                        </motion.span>
                        <motion.div animate={{ rotate: favoritesOpen ? 0 : -90 }} transition={{ duration: 0.2 }}>
                          <ChevronLeft className={cn("w-4 h-4 transition-transform duration-200", favoritesOpen ? "-rotate-90" : "rotate-0")} />
                        </motion.div>
                      </>
                    )}
                  </AnimatePresence>
              ...
              {isCollapsed && <TooltipContent side="right" className="font-medium">Favorites</TooltipContent>}
            </Tooltip>
          ...
          <AnimatePresence mode="wait">
            {!isCollapsed && (
              <motion.div
                ...
                exit={{ opacity: 0, height: 0 }}
                transition={{ duration: 0.2 }}
                className="p-3 rounded-xl bg-gradient-to-br from-[rgba(139,92,246,0.1)] to-[rgba(245,158,11,0.05)] border border-[rgba(139,92,246,0.2)]"
              >
                <p className="text-xs text-muted-foreground">Mog Dashboard</p>
              </motion.div>
            )}
          </AnimatePresence>

          <div className={cn("flex items-center", isCollapsed ? "flex-col gap-2" : "justify-center")}>
            <Tooltip delayDuration={0}>
              <TooltipTrigger asChild>
                <button
                  className={cn(
