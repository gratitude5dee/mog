...
export function useOnboardingTour() {
  ...
  const start = useCallback(() => { setCurrentStepIndex(0); setIsActive(true); }, []);
  const stop = useCallback(() => { setIsActive(false); setCurrentStepIndex(0); }, []);
  ...
  return { isActive, currentStep, currentStepIndex, totalSteps, hasCompleted, start, stop, complete, next, prev, goToStep };
}
